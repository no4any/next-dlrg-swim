import { Distance } from "@/src/components/Distance.component";
import { Medal } from "@/src/components/Medal.component";
import { dateToGermanDate, getAge, getGenderString } from "@/src/lib";
import { getDistance } from "@/src/lib/getDistance";
...
export async function SwimmerList({ swimmers, noTeam }: { noTeam?: boolean, swimmers: (Swimmer & { team?: Team })[] }) {
    return <div className="grid grid-col-5 gap-1">
        <div className="flex flex-row gap-0-5 p-1 font-bold sticky">
            ...
            <div className="flex-3 hidden md:block">Geschlecht</div>
            <div className="flex-3 hidden md:block">Geburtstag</div>
            <div className="flex-3">Alter</div>
        </div>
                ...
                <div>
                    <div className="flex flex-row gap-0.5 hover:bg-gray-200 rounded-md p-1">
                        <div className="flex-1 flex-row">
                            <Medal distance={await getDistance(swimmer)} swimmer={swimmer} />
                        </div>
                        ...
                        <div className="flex-3 hidden md:block">{swimmer.gender && getGenderString(swimmer.gender)}</div>
                        <div className="flex-3 hidden md:block">{birthday && dateToGermanDate(birthday)}</div>
                        <div className="flex-3">{birthday && getAge(birthday)}</div>
                    </div>
                </div>
            </Link>
        }))}
